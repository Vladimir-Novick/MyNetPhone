...
RequestDlg rdlg(NULL);
	...
	if(type==TYPE_CONTROL)
	{
		...
		switch(data[0])
		{
			...
			( (CMyNetPhoneClientDlg*)pdlg)->DestroyConference();
			...
			( (CMyNetPhoneClientDlg*)pdlg)->StartConference();
			return;
	...
	if(type==TYPE_AUDIO)
	{
			...
			// Play the audio
			((CMyNetPhoneClientDlg *)pdlg)->play->PostThreadMessage(WM_PLAYSOUND_PLAYBLOCK,retvalue,(LPARAM)adata);
	...
	if(type==TYPE_VIDEO)
	{
			...
			((CMyNetPhoneClientDlg *)pdlg)->DisplayRemoteFrame(vdata,retvalue);
	...
	if(type==TYPE_TEXT)
	{
		...
		((CMyNetPhoneClientDlg *)pdlg)->DisplayMesg(str);
